...
import { createResponsiveSystem, ScreenClassConfiguration } from '../../..';
...
export const SM_OVERRIDE_TEXT = 'sm';
export const MD_OVERRIDE_TEXT = 'md';
...
export function makeScreenClassTester(
  ...
  const ResponsiveComp: React.FC<{}> = () => {
    const responsiveValue = useResponsiveValue(DEFAULT_TEXT, {
      sm: SM_OVERRIDE_TEXT,
      md: MD_OVERRIDE_TEXT,
    });
    return <div data-testid="value-hook-comp">{responsiveValue}</div>;
  };

  return function testScreenClass(
    screenClass: keyof typeof breakpoints,
    expected: keyof typeof breakpoints | 'default',
  ) {
    ...
    expect(getByTestId('value-hook-comp').innerHTML).toBe(expected);
  };
}
